...
++ asi: Fixture 'luft|hastighets|indikator+n; lufthastighet+en; siffror+na asi+n'
    "Den mäter för närvarande en flyghastighet på <<airspeed>> knop. 
...
//        <<asi.airspeed>> knots. ";   
        ...
        if(wheel.angle != 0 && asi.airspeed > 0)
        {
            ...
            med ett stillastående flygplan <<or>> och kör in i en grupp skjul 
        ...
        if(asi.airspeed > oldSpeed && oldSpeed == 0)
            "Flygplanet börjar röra sig framåt. ";
        else if (asi.airspeed > oldSpeed)
            "Flygplanet forsätter att öka farten. ";
        if(asi.airspeed < oldSpeed && asi.airspeed == 0)
            "Flygplanet stannar. ";
        else if(asi.airspeed < oldSpeed)
            "Flygplanet tappar fart. ";
    }   
;
...
bathroom: Room 'Toaletten' 'toalett+en;;toa+n wc bås+et'
...
disembarkingPassengers: Decoration 'missnöjd+a passagera:re+na; 
